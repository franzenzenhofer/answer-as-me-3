#!/bin/bash
# Single-file deployment script for Answer As Me 3
...
DESCRIPTION:
    This script builds and deploys the Answer As Me 3 add-on to
...
if [[ "$DRY_RUN" == true ]]; then
    ...
    if command -v npm &> /dev/null && [[ -f "package.json" ]]; then
        npm run test:postbundle || echo "⚠️ Post-bundle tests not configured yet"
    fi
    echo "✅ Post-bundle validation completed"
...
if [[ "$DRY_RUN" == true ]]; then
    ...
    echo "${DRY_RUN_PREFIX}Would run: clasp deploy --description \"v$VERSION - Single File Bundle - Hello World\""
    ...
    DEPLOY_DESC="v$VERSION - Single File Bundle - Hello World"
